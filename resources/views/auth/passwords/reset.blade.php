...
@extends('layouts.frontend', ['title' => trans('adminlte.auth.reset.title'), 'class' => 'homepage', 'header' => false])
            ...
            <div class="row">
                <div class="auth-form col-md-6 col-sm-12">
                    <div class="d-flex align-items-left justify-content-left mb-3 mb-lg-4">
                        <img class="img-h75 mb-0" src="{{ asset('images/logo.png') }}" alt="@lang('adminlte.auth.reset.info')">
                    </div>
                    <h2 class="text-primary font-weight-600 mb-3">
                        @lang('adminlte.auth.reset.title')
                    </h2>
                    <p class="font-weight-500 lead font-17">
                        @lang('adminlte.auth.reset.info')
                    </p>

                    {{ BsForm::resource('customers')->post(route('password.update')) }}
                        {{ BsForm::hidden('token')->value($token) }}
                        {{ BsForm::email('email')->value($email ?? old('email')) }}
