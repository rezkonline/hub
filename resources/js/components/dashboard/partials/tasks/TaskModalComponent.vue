<template>
    <modal name="task" width="90%" height="100%" :adaptive="true" :scroll="true" @before-open="beforeOpenTaskModal" styles="background: transparent;box-shadow: none;margin-top: 1.75rem;" classes="modalsingleaelan v--modal">
        <a class="closefafa fa fa-times" @click="$modal.hide('task')" href="#"></a>
                        ...
                        <div class="block__box--content p-2 p-md-3 p-lg-4">
                            ...
                            <div class="row row-p8 row-col-lg mb-4">
                                ...
                                <div class="col-lg-12 text-center mt-3">
                                    <button class="btn btn-secondary btn-mw-full btn-md-mw150 rounded btn-lg mb-2" style="color: #fff;" @click="$modal.show('change-status-task', { task: task })">{{ $t('frontend.change_status') }}</button>
                                </div>
...
<script>
    export default {
        ...
        methods: {
            async beforeOpenTaskModal(event) {
                var app = this;
            ...
            getComments() {
                var app = this;
                this.axios.get('tasks/' + this.task.id + '/comments').then(function (response) {
            ...
            getAttachments() {
                var app = this;
                this.axios.get('tasks/' + this.task.id + '/attachments').then(function (response) {
            ...
            addComment() {
                ...
                this.axios.post('tasks/' + this.task.id + '/comment', {
                    body: this.comment
                }).then(() => {
                    app.getComments();
                });
            ...
            getLogs() {
                var app = this;
                this.axios.get('tasks/' + this.task.id + '/logs').then((response) => {
