<template>
    <modal name="schedule" width="90%" height="100%" :adaptive="true" @before-open="beforeOpenScheduleModal" styles="background: transparent;box-shadow: none;margin-top: 1.75rem;" classes="modalsingleaelan v--modal">
        <a class="closefafa fa fa-times" @click="$modal.hide('schedule')" href="#"></a>
                        ...
                        <div class="block__box--content p-2 p-md-3 p-lg-4">
                            ...
                            <div class="row row-p8 row-col-lg mb-4">
                                ...
                                <div class="col-lg-12 text-center mt-3">
                                    <button class="btn btn-secondary btn-mw-full btn-md-mw150 rounded btn-lg mb-2" style="color: #fff;" @click="$modal.show('change-status-schedule', { schedule: schedule })">{{ $t('frontend.change_status') }}</button>
                                </div>
...
<script>
    export default {
        ...
        methods: {
            ...
            getComments() {
                var app = this;
                this.axios.get('schedules/' + this.schedule.id + '/comments').then(function (response) {
            ...
            getAttachments() {
                var app = this;
                this.axios.get('schedules/' + this.schedule.id + '/attachments').then(function (response) {
            ...
            addComment() {
                ...
                this.axios.post('schedules/' + this.schedule.id + '/comment', {
                    body: this.comment
            ...
            getLogs() {
                var app = this;
                this.axios.get('schedules/' + this.schedule.id + '/logs').then((response) => {
